...
export default async function StudentsPage({
  ...
  searchParams: Promise<{ search?: string; status?: string; batch?: string }>
}) {
  const { search, status, batch } = await searchParams
    ...
    <div className="space-y-8 max-w-[1200px]">
      ...
      <form method="GET" className="flex gap-3 flex-wrap">
        ...
        {(search || status) && (
          <Link
            href="/students"
            className="h-10 px-4 flex items-center text-sm font-medium text-slate-500 hover:text-slate-800 transition-colors"
          >
            Clear
          </Link>
        )}
      ...
      {students.length === 0 ? (
        <div className="bg-white border border-slate-200/50 rounded-2xl shadow-sm p-16 text-center">
          ...
          <p className="text-xs font-medium text-slate-400 mt-1">
            {search || status ? "Try adjusting your filters" : "Enroll the first student to get started"}
          </p>
        </div>
      ) : (
