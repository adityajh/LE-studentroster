...
export function EnrollForm({ batches }: { batches: Batch[] }) {
  ...
  const [installmentType, setInstallmentType] = useState<"ANNUAL" | "ONE_TIME">("ANNUAL")
  ...
  const fees = useMemo(() => {
    ...
    const annualSchedule = [
      { label: "Registration Fee", amount: reg, due: "Today" },
      { label: "Year 1 — Growth", amount: Math.max(0, y1 - waiverPerYear), due: `Jul 1, ${selectedBatch!.year}` },
      { label: "Year 2 — Projects", amount: Math.max(0, y2 - waiverPerYear), due: `Jul 1, ${selectedBatch!.year + 1}` },
      { label: "Year 3 — Work", amount: Math.max(0, y3 - waiverPerYear), due: `Jul 1, ${selectedBatch!.year + 2}` },
    ]

    const oneTimeSchedule = [
      { label: "Registration Fee", amount: reg, due: "Today" },
      { label: "Full Programme Fee (3 Years)", amount: Math.max(0, y1 + y2 + y3 - totalWaiver), due: "Within 30 days" },
    ]
      ...
      {fees && (
        <div className="bg-white border border-slate-200/50 rounded-2xl shadow-sm p-6 space-y-5">
          ...
          <div className="flex gap-3">
            {(["ANNUAL", "ONE_TIME"] as const).map((type) => (
              <button
                key={type}
                type="button"
              ...
              >
                {type === "ANNUAL" ? "Annual (3 years)" : "One-Time (full payment)"}
              </button>
            ))}
          </div>
